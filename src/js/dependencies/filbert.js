...
(function(root, mod) {
  ...
  function parseStatement() {
    ...
    case _global:                                   // ## OpenSesame Addition
      next();
      return parseGlobal(node);
    case _if: case _elif:
      next();
  ...
  // Parse global ## osweb addition
  
  function parseGlobal(node) {
    node.kind = 'var';
    node.declarations = [];
    var done = false;
    while (done === false) {
        ...
        finishNode(node, decl, "VariableDeclarator");

        node.declarations.push(decl);
          
        // If token is comma skip it.
        if (tokType === _comma) {
            next();
        }   
        
        done = (tokType === _newline) || (tokType === _eof); 
    }
  ...
  // ## Python runtime library
    ...
    var pythonRuntime = exports.pythonRuntime = {
        ...
        utils: {
            // Convert an dict object to a python dict representation (see interpreter.pdc for supported properties/methods).
            convertToDict: function (dict) {
                ...
                if (!dict.hasOwnProperty("copy")) {
                    Object.defineProperty(dict, "copy", {
                        value: function () {
                            var new_copy = filbert.pythonRuntime.objects.dict();
                            for (var key in this) {
                                new_copy[key] = this[key];
                            }
                            return new_copy;
                        },
                        enumerable: false
                ...
                if (!dict.hasOwnProperty("update")) {
                    Object.defineProperty(dict, "update",
                    {
                        value: function (dict2) {
      ...
      createList: function () {
        ...
        if (arguments.length === 1 && arguments[0] instanceof Array) {
           for (var i=0;i < arguments[0].length;i++) ret.push(arguments[0][i]);} // ### OSWEB HACK
        else {
    ...
    ops: {
      ...
      subscriptIndex: function (o, i) {
        ...
        // Definition of the build in objects.
        objects: {
            dict: function () {
                var obj = {};
        ...
        // Definition of the build in functions.
        functions: {
            abs: function(x) {
                return Math.abs(x);
            },
            ...
            print: function () {
                ...
                for (var i = 0; i < arguments.length; i++) {
                    ...
                    s += i === 0 ? JSON.stringify(arguments[i]) : " " + JSON.stringify(arguments[i]);
                    }
                    else {
        ...
        // Placeholder for the import classes and functions.
        imports: {
        }
    };
});
